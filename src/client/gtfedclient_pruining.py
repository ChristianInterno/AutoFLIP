...
class GtfedClient(BaseClient):
    ...
    def create_scout(self, model):
        if not checkpoint_exists('tl_base_pruining'):
            print('The explorers are getting ready to explore the different loss landscape!')
            tl_base = model.train()
        ...
        if not mask_exists(f'mask{self.args.exp_name}'):
            print(f'The client {self.id} is exploring...')
            ...
            for e in range(self.args.epoochs_mask):
                ...
                print(
                    f'Exploration epoch: {e + 1}, Loss: {loss.item()}, Accuracy: {epoch_accuracy}')  # print the epoch number, loss, and accuracy
                ...
                if patience_counter >= patience:  # if counter reaches the threshold
                    print(f'Early stopping at epoch {e + 1}, best exploration loss was {best_loss}')
    ...
    # In utils.py or at the top of both gtfedclient.py and gtfedserver.py
